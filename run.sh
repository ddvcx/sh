...
DEVICE_AX6_1="qualcommax" #master
DEVICE_AX6_2="ipq807x" #23.05
REP_LEDE="LEDE"
...
Update(){
	...
	sudo apt -y update
	sudo apt -y upgrade
	sudo apt -y purge azure-cli ghc* llvm* firefox google* openjdk* mysql* php* android*
	sudo apt -y install curl git git-core wget nano net-tools upx-ucl
	for pkg in libtool libtool-bin cmake gperf gawk bison xxd cpio gettext help2man pkg-config libgmp3-dev libmpc-dev libmpfr-dev libltdl-dev build-essential asciidoc binutils bzip2 libncurses5-dev libz-dev patch unzip zlib1g-dev libc6-dev-i386 subversion flex uglifyjs gcc-multilib p7zip p7zip-full rsync msmtp libssl-dev texinfo libglib2.0-dev xmlto qemu-utils libelf-dev autoconf automake  autopoint device-tree-compiler psmisc iptables jq netcat-openbsd screen socat ; do
		sudo apt -y install "$pkg" || echo "跳过失败: $pkg"
	done
...
DIY() { #修改配置文件
	...
	F=${DIR}/${REP}/target/linux/${DEVICE_AX6}/Makefile #软件包
	sed -i 's@ramdisk @@g' ${F}
	sed -i 's@htop @@g' ${F}
...
COPY_AX6(){
	...
	if ls ${DIR_ROM_AX6}/*${DEVICE_AX6}*.bin 1> /dev/null 2>&1; then
		echo "编译成功"
		mv -f ${DIR_ROM_AX6}/*${DEVICE_AX6}*.bin ${DIR}/${REP}-ax6-${DATE}.bin
		mv -f ${DIR_ROM_AX6}/*${DEVICE_AX6}*.ubi ${DIR}/${REP}-ax6-${DATE}.ubi
		cp -fr ${DIR}/${REP}/${CFG} ${DIR}/${CFG_AX6}
	else
		echo "编译失败"
	fi
}
...
MAKE_AX6(){
	cd ${DIR}/${REP}
	if ls ${CFG_AX6} 1> /dev/null 2>&1; then
		echo "复制配置文件"
		cp -fr ${CFG_AX6} ${CFG}
	else
		echo "未找到配置文件"
...
Immortal() {
	...
	TREE="${TREE_Immortal}"
	DEVICE_AX6="${DEVICE_AX6_1}" #master
	#DEVICE_AX6="${DEVICE_AX6_2}" #23.05
	DIR_PACKAGE="${DIR}/${REP}/package"
	DIR_ROM_AX6="${DIR}/${REP}/bin/targets/${DEVICE_AX6}/${DEVICE_AX6_2}" #master
	#DIR_ROM_AX6="${DIR}/${REP}/bin/targets/${DEVICE_AX6}/generic" #23.05
	LINE
	select opt in 克隆源码 更新代码/修改设置 配置参数 更新代码/编译 编译/单线程 修改目录权限 删除目录;
    do case $opt in
        克隆源码)
			CLONE
			DIY
